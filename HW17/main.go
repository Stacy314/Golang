...
func getTasks(c echo.Context) error {
	...
	if err == redis.Nil {
		...
		err = rdb.Set(ctx, cacheKey, tasksJSON, 5*time.Minute).Err() // Cache timeout - 5 хвилин
		if err != nil {
			return c.JSON(http.StatusInternalServerError, echo.Map{"error": "Error saving to cache"})
		}
...
func addTask(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": "Error clearing cache"})
	}
